        ...
        <div>
            ...
            @if ($advertisements->count() > 0)
                @foreach ($advertisements as $advertisement)
                    <div class="border rounded-md p-4 mb-4 container mx-auto">
                        ...
                        <div id="deleteAdvertisementModal-{{ $advertisement->id }}" class="fixed inset-0 flex items-center justify-center z-50 hidden">
                            <div class="bg-white w-1/3 p-6 rounded-lg">
                                ...
                                <div class="flex justify-end mt-6">
                                    <button id="cancelDelete-{{ $advertisement->id }}" class="px-4 py-2 bg-gray-500 text-white rounded-md mr-2">{{__('messages.cancel')}}</button>
                                    <form id="deleteAdvertisementForm-{{ $advertisement->id }}" action="{{ route('advertisements.destroy', ['id' => $advertisement->id]) }}" method="POST">
                                        @csrf
                                        @method('DELETE')
    ...
    <script>
        ...
        deleteButtons.forEach(button => {
            button.addEventListener('click', () => {
                ...
                const deleteAdvertisementForm = document.getElementById(`deleteAdvertisementForm-${advertisementId}`);
                const deleteAdvertisementModal = document.getElementById(`deleteAdvertisementModal-${advertisementId}`);

                // Show the delete modal
                deleteAdvertisementModal.classList.remove('hidden');

                // Cancel delete action
                const cancelDeleteButton = document.getElementById(`cancelDelete-${advertisementId}`);
